...
class EasyFloat {
    ...
    companion object {
        ...
        @JvmOverloads
        @JvmStatic
        fun with(activity: Context, ace: AccessibilityService?=null): Builder = if (activity is Activity) Builder(activity,ace)
        else Builder(LifecycleUtils.getTopActivity() ?: activity,ace)
        ...
        /**
         * 设置当前浮窗是否横屏，先获取浮窗的config，后修改相应属性
         * @param dragEnable    是否横屏
         * @param tag           浮窗标签
         */
        @JvmStatic
        @JvmOverloads
        fun forceLandscape(isLandscape: Boolean = true, tag: String? = null) =
            getConfig(tag)?.let { it.forceLandscape = isLandscape }
     ...
     * 浮窗的属性构建类，支持链式调用
     */
    class Builder(private val activity: Context,private  val ace:AccessibilityService?) : OnPermissionResult {
         ...
         */
        fun setLandscape(isLandscape: Boolean = true) = apply { config.forceLandscape = isLandscape }
         ...
         * 通过浮窗管理类，统一创建浮窗
         */
        private fun createFloat() = FloatingWindowManager.create(activity, config,ace)
