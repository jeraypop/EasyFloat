 ...
 */
internal class FloatingWindowHelper(val context: Context, var config: FloatConfig,val ace:AccessibilityService?) {
    ...
    private fun initParams() {
        if (ace==null){
            ...
            Log.e("悬浮窗", "无障碍 wm: " )
            windowManager = ace?.getSystemService(WindowManager::class.java)as WindowManager
//            windowManager = ace?.getSystemService(Service.WINDOW_SERVICE) as WindowManager
        }
...
//        windowManager = context.getSystemService(Service.WINDOW_SERVICE) as WindowManager
            ...
            if (config.showPattern == ShowPattern.CURRENT_ACTIVITY) {
               ...
               if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    if (ace!=null){
                        ...
                        Log.e("悬浮窗", "ACCESSIBILITY: " )
                    }else{
                        type = TYPE_APPLICATION_OVERLAY
                        Log.e("悬浮窗", "APPLICATION: " )
                    }
    ...
    private fun setGravity(view: View?) {
        ...
        // 获取状态栏高度，如果浮窗的顶部坐标大于当前的 Y 坐标，则说明浮窗跨越了状态栏
        val statusBarHeight = if (location[1] > params.y) DisplayUtils.statusBarHeight(view) else 0
        Log.e("状态栏高度", "setGravity: $statusBarHeight")

        val parentBottom = config.displayHeight.getDisplayRealHeight(context) - statusBarHeight
